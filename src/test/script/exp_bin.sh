...
gen_exp_part () {
	...
	echo "//    	Ligne 11: L’opérande gauche est de type $args2, L’opérande droite est de type $args3. Or les opérandes d’un modulo doivent être de type ‘int’" >&3
	echo "">&3
	echo "{" >&3
	echo "$args2 a;" >&3
	echo "$args3 b;" >&3
	echo "int c = a ""$args4"" b;" >&3
	echo "}" >&3
	exec 3>&-
}
